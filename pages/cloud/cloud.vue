	...
	export default {
		...
		methods: {
			...
			getFileName(e) {
				...
				uni.request({
					url: 'http://' + ip + ':5244/api/fs/list', // 请求的接口地址
					method: 'POST', // 请求方法
					data: data,
					header: {
						'Accept': 'application/json, text/plain, */*',
						'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8',
						'Authorization': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6IjY2NiIsInB3ZF90cyI6MTcxNjkwMDg1NSwiZXhwIjoxNzQxNzUwNjc0LCJuYmYiOjE3NDE1Nzc4NzQsImlhdCI6MTc0MTU3Nzg3NH0.K6FVri_oAYL8vA2YNM7WzaXgc4J5m0VH3Fsy3KZoKsE',
						'Cache-Control': 'no-cache',
						'Connection': 'keep-alive',
						'Content-Type': 'application/json;charset=UTF-8',
						'Origin': 'http://' + ip + ':5244',
						'Pragma': 'no-cache',
						'Referer': 'http://' + ip + ':5244/%E5%A4%B8%E5%85%8B%E7%BD%91%E7%9B%98',
					...
					success: (res) => {
						this.root = res.data.data; // 存储返回的数据
						console.log(res.data.data.content);

						// 正则表达式匹配影视名称和年份
						const pattern =
							/[A-Z]?[\s\.]*(.*?)(?:1080P|BluRay|WEBDL|HDTV|\.mp4|\.mkv)?\s*[\(\（](\d{4})[\)\）]/;
						const results = [];
						...
						for (const item of res.data.data.content) {
							...
							if (match) {
								// 提取中文影视名字
								const movieName = match[1].replace(/[^\u4e00-\u9fa5]+/g, "").trim();
								const year = match[2];
								if (year) { // 确保年份不为空
									results.push(`${movieName}.${year}`);
								}
							}
						}
						// 打印匹配结果
						...
						console.log(this.path.includes("漫威电影"))
						const arrayString = JSON.stringify(results);
						uni.navigateTo({
							url: '/pages/movie/movie?results=' + arrayString // 目标页面路径
						});
					},
					fail: (err) => {
			...
			getfile(e) {
				...
				uni.request({
					url: 'http://' + ip + ':5244/api/fs/get', // 请求的接口地址
					method: 'POST', // 请求方法
					data: data,
					header: {
						'Accept': 'application/json, text/plain, */*',
						...
						'Pragma': 'no-cache',
						'Referer': 'http://' + ip +
							':5244/%E5%A4%B8%E5%85%8B%E7%BD%91%E7%9B%98/%E5%89%A7%E9%9B%86/%E4%BA%BA%E7%94%9F%E5%88%87%E5%89%B2%E6%9C%AF',
						'Sec-Fetch-Dest': 'empty',
						'Sec-Fetch-Mode': 'cors',
						'Sec-Fetch-Site': 'same-origin',
						'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36',
